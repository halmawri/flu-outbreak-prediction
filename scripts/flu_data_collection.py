...
def fetch_cdc_ili_data(start_year=2017, end_year=2024):
    ...
    # Convert years to epiweeks (format YYYYWW)
    start_epiweek = int(f"{start_year}01")  # First week of start_year
    end_epiweek = int(f"{end_year}53")      # Last possible week of end_year
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            # Process the data
            print(f"Received {len(data.get('epidata', []))} records from the API")
            ...
            if not df.empty:
                ...
                print(f"Successfully fetched CDC ILI data: {len(df)} records.")
                return df
            else:
                print("No CDC ILI data retrieved.")
                return None
        else:
        ...
        print(f"Error fetching CDC ILI data: {str(e)}")
        return None

# Alternative function using CDC's direct API (as a backup if the above function fails)
def fetch_cdc_ili_data_alt(start_year=2017, end_year=2024):
    """
    Alternative method to fetch CDC ILI data using CDC's FluView API.
    ...
    pandas.DataFrame
        A DataFrame containing the CDC ILI data.
    """
    print(f"Fetching CDC ILI data from {start_year} to {end_year} using alternative CDC API...")
    
    # CDC FluView API endpoint for ILI activity
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            # Process the data (mapping depends on CDC's XML structure)
            # This is a simplified example - actual XML structure may vary
            for item in root.findall('.//ILIActivity'):
                week_data = item.find('week')
                if week_data is not None:
                    year_str = week_data.find('year').text
                    week_str = week_data.find('week').text
                    ...
                    if year_str and week_str:
                        ...
                        if start_year <= year <= end_year:
                            ...
                            try:
                                ...
                                # Handle weeks beyond the standard calendar
                                date = datetime(year, 12, 28).date()
                            
                            # Extract data (adjust based on actual XML structure)
                            ili_percent = float(item.find('percent').text) if item.find('percent') is not None else None
                            total_patients = int(item.find('patients').text) if item.find('patients') is not None else None
            ...
            print(f"Error: Unable to fetch CDC ILI data (alternative method). Status code: {response.status_code}")
            return None
            
    except Exception as e:
        print(f"Error fetching CDC ILI data (alternative method): {str(e)}")
        return None
